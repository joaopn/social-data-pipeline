"""
CSV to PostgreSQL ingestion for social_data_pipeline.
"""
        ...
        with conn.cursor() as curr:
            for ts_name in tablespaces:
                ...
                if curr.fetchone() is None:
                    ...
                    print(f"[sdp] Created tablespace: {ts_name} -> {location}")
                else:
                    print(f"[sdp] Tablespace exists: {ts_name}")
...
def analyze_table(
    ...
    print(f"[sdp] ANALYZE complete: {full_table}")
...
def delete_duplicates(
    ...
    print(f"[sdp] Removing duplicates from {full_table}...")
    ...
    print(f"[sdp] Deleted {deleted_count} duplicate rows from {full_table}")
...
def finalize_fast_load_table(
    ...
    with _connect(dbname, user, host, port, password) as conn:
        with conn.cursor() as curr:
            ...
            conn.commit()
    print(f"[sdp] PRIMARY KEY added to {full_table}")
        ...
        if table_exists(fk_reference_table, schema, dbname, host, port, user, password):
            ...
            with _connect(dbname, user, host, port, password) as conn:
                with conn.cursor() as curr:
                    ...
                    conn.commit()
            print(f"[sdp] FOREIGN KEY added to {full_table}")
        else:
            print(f"[sdp] Warning: Reference table {ref_table} not found, skipping FK constraint")
...
def create_fast_load_table(
    ...
    print(f"[sdp] Creating table {schema}.{table} (no PK){ts_label}...")
    ...
    print(f"[sdp] Created table {schema}.{table} (no PK)")
...
def create_fast_load_classifier_table(
    ...
    """
    ts_label = f" on tablespace {tablespace}" if tablespace else ""
    print(f"[sdp] Creating table {schema}.{table_name} (no PK, no FK){ts_label}...")
    ...
    print(f"[sdp] Created table {schema}.{table_name} (no PK, no FK)")
...
def create_index(
    ...
    print(f"[sdp] Creating index: {index_name}")
    ...
    with _connect('postgres', user, host, port, password, autocommit=True) as conn:
        with conn.cursor() as curr:
            ...
            if curr.fetchone() is None:
                ...
                print(f"[sdp] Created database: {dbname}")
...
def ingest_csv(
    ...
    print(f"[sdp] Starting ingestion: {csv_file} -> {schema}.{table}")
...
def get_table_column_list(
    ...
    try:
        ...
        print(f"[sdp] Warning: Could not query columns for {table_name}: {e}")
...
def ingest_classifier_csv(
    ...
    print(f"[sdp] Starting ingestion: {csv_file} -> {schema}.{table_name}")
    ...
    if not table_exists(table_name, schema, dbname, host, port, user, password):
        ...
        print(f"[sdp] Inferring schema from {type_inference_rows} rows...")
        column_list, column_types, nullable_cols = infer_classifier_schema(
            csv_file, type_inference_rows, column_overrides
        )
        
        if nullable_cols:
            print(f"[sdp] Columns with empty values (using FORCE_NULL): {nullable_cols}")
        
        print(f"[sdp] Inferred columns: {column_list}")
        ...
        if use_foreign_key and not main_table_exists:
            print(f"[sdp] Warning: Main table {schema}.{data_type} not found, creating without FK")
        ...
        print(f"[sdp] Created table: {schema}.{table_name}{fk_status}")
    else:
    ...
    print(f"[sdp] Ingestion complete: {schema}.{table_name}")
