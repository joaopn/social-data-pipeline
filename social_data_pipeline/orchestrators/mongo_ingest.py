"""
MongoDB ingestion profile orchestrator for social_data_pipeline.
...
def run_pipeline(config_dir: str = "/app/config"):
    ...
    for dt in data_types:
        ...
        if states[dt].get_stats()['processed_count'] == 0:
            print(f"[sdp] No state for {dt}, attempting to recover from MongoDB...")
    ...
    if skipped_count > 0:
        print(f"[sdp] Skipping {skipped_count} already extracted/processed compressed files")
        ...
        for db_name, dt_collections in collections_by_db.items():
            for data_type, collection_names in dt_collections.items():
                ...
                for collection_name in sorted(collection_names):
                    print(f"[sdp] Creating {len(index_fields)} indexes on {db_name}.{collection_name}")
