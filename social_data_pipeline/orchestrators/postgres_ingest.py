"""
PostgreSQL database profile orchestrator for social_data_pipeline.
...
def run_pipeline(config_dir: str = "/app/config"):
    ...
    for dt in data_types:
        ...
        if states[dt].get_stats()['processed_count'] == 0:
            print(f"[sdp] No state for {dt}, attempting to recover from database...")
    ...
    print(f"\n[sdp] Found {len(files)} .zst files in {dumps_dir}")
    ...
    if skipped_count > 0:
        print(f"[sdp] Skipping {skipped_count} already processed .zst files")
    ...
    print(f"\n[sdp] Mode: {'parallel' if parallel_mode else 'sequential'}")
        ...
        for filepath, data_type in pending_zst_files:
            ...
            if not expected_json.exists():
                try:
                    ...
                    print(f"[sdp] Error extracting {file_id}: {e}")
    ...
    if files_to_parse:
        ...
        if parallel_mode and len(files_to_parse) > 1:
            print(f"[sdp] Parallel mode: {len(files_to_parse)} files with {workers} workers")
                ...
                if cleanup_temp:
                    for json_path, file_id, _ in files_to_parse:
                        if os.path.exists(json_path):
                            os.remove(json_path)
                            print(f"[sdp] Removed: {Path(json_path).name}")
            except Exception as e:
            ...
            for json_path, file_id, data_type in files_to_parse:
                try:
                    ...
                    print(f"[sdp] Error parsing {file_id}: {e}")
                    states[data_type].mark_failed(file_id, f"Parsing failed: {e}")
                    fail_count += 1
    ...
    if files_to_ingest:
        ...
        if prefer_lingua and parsed_source_map:
            ...
            print(f"[sdp] Sources: {ingest_from_lingua} lingua, {ingest_from_original} original (fallback)")
        ...
        if standard_load_types:
            print(f"[sdp] Standard ON CONFLICT load for: {', '.join(sorted(standard_load_types))}")
            ...
            def fast_load_data_type(data_type):
                ...
                for parsed_path, file_id, dt in type_files:
                    try:
                        ...
                        print(f"[sdp] Error during COPY {file_id}: {e}")
            ...
            if use_parallel_fast_load:
                ...
                with ThreadPoolExecutor(max_workers=2) as executor:
                    ...
                    try:
                        ...
                        print("[sdp] Database may be in inconsistent state. Manual recovery required.")
                        raise
            else:
                for data_type in sorted(fast_load_types):
                    try:
                        ...
                        print(f"[sdp] CRITICAL ERROR for {data_type}: {e}")
                        print("[sdp] Database may be in inconsistent state. Manual recovery required.")
                        raise
        ...
        if standard_load_types:
            ...
            if use_parallel_ingestion:
                print("[sdp] Parallel ingestion enabled")
    ...
    if create_indexes and success_count > 0:
        ...
        for data_type in data_types:
            ...
            print(f"[sdp] Creating {len(index_fields)} indexes on {db_config['schema']}.{data_type} (workers_per_build={parallel_index_workers})")
