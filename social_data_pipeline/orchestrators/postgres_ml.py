"""
PostgreSQL ML profile orchestrator for social_data_pipeline.
...
def run_pipeline(config_dir: str = "/app/config"):
    ...
    for classifier_name, classifier_cfg in classifiers_config.items():
        ...
        if standard_load_types:
            print(f"[sdp] Standard ON CONFLICT for: {', '.join(sorted(standard_load_types))}")
        ...
        if fast_load_types:
            ...
            def fast_load_classifier_type(dt):
                ...
                print(f"\n[sdp] Fast loading {table_name}: {len(type_files)} files")
